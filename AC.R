...
train_df = ovun.sample(Reseller ~ ., data = train_df, method = "under", p = 0.5, seed = 5)$data
...
logistic_model <- glm(Reseller ~ ., 
                      data = train_df, 
                      family = "binomial")

summary(logistic_model)

MCFad = 1 - (logistic_model$deviance / logistic_model$null.deviance)

preds = predict(logistic_model, newdata = test_df, type = 'response')

test_df = test_df %>% mutate(pred = round(preds))
test_df %>% group_by(Reseller, pred) %>% summarise(count = n())
